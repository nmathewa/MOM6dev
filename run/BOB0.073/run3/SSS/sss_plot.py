...
sss_run_2 = xr.open_dataset("~/HDD2/runs/run2/SSS/sss_model.nc")
sss_run_3 = xr.open_dataset("~/HDD2/runs/run3/SSS/sss_model.nc")
...
def main_plot(ti):
    ...
    sss_run2 = sss_run_2["SSS"]
    sss_run3 = sss_run_3["SSS"]
    ...
    diff2 = sss_obs - sss_run2 
    diff3 = sss_obs - sss_run3
    ...
    ax3 = fig.add_subplot(grid[0, 2], projection=proj)  # lower cell of grid
    ...
    ax7 = fig.add_subplot(grid[2, 0], projection=proj)  # upper cell of grid
    ax8 = fig.add_subplot(grid[2, 1], projection=proj)  # middle cell of grid
    ax9 = fig.add_subplot(grid[2, 2], projection=proj)  # lower cell of grid

    for (ax, title) in [(ax1, t1), (ax2, t1), (ax3, t1), (ax4, t1), (ax5, t1), (ax6, t1), (ax7, t1), (ax8, t1), (ax9, t1)]:
    ...
    ax5.set_title("Model_TRMM", loc='center', y=1.04, fontsize=10)
    ax6.set_title("Obs - Model_TRMM", loc='center', y=1.04, fontsize=10)
    ax8.set_title("Model_TRMM_ASCAT", loc='center', y=1.04, fontsize=10)
    ax9.set_title("Obs - Model_TRMM_ASCAT", loc='center', y=1.04, fontsize=10)
    ...
    ax2.contourf(sss_run['xh'],
                 ...
                 cmap=cmap,
                 extend='both')
    ax4.contourf(sss_obs['xh'],
                 sss_obs['yh'],
                 sss_obs,
                 levels=np.arange(tmin, tmax, .25),
                 cmap=cmap,
                 extend='both')
    ax5.contourf(sss_run2['xh'],
                 sss_run2['yh'],
                 sss_run2,
                 levels=np.arange(tmin, tmax, .25),
                 cmap=cmap,
                 extend='both')
    ax7.contourf(sss_obs['xh'],
                 sss_obs['yh'],
                 sss_obs,
                 levels=np.arange(tmin, tmax, .25),
                 cmap=cmap,
                 extend='both')
    ax8.contourf(sss_run3['xh'],
                 sss_run3['yh'],
                 sss_run3,
                 levels=np.arange(tmin, tmax, .25),
                 cmap=cmap,
    ...
    C_2 = ax3.contourf(diff['xh'],
                       ...
                       cmap="RdBu_r",
                       extend='both')
    ax6.contourf(diff2['xh'],
                       diff2['yh'],
                       diff2,
                       levels=np.arange(-5, 5, 1),
                       cmap="RdBu_r",
                       extend='both')
    ax9.contourf(diff3['xh'],
                       diff3['yh'],
                       diff3,
                       levels=np.arange(-5, 5, 1),
                       cmap="RdBu_r",
