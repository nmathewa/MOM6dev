...
sst_run_2 = xr.open_dataset("~/HDD2/runs/run2/SST/sst_model.nc")

def main_plot(ti):
    # data
    sst_obs = sst_obs_file["sst"]
    sst_run = sst_run_file["SST"]
    sst_run2 = sst_run_2["SST"]

    # select time 
    sst_obs = sst_obs.isel(time=ti)
    sst_run = sst_run.isel(time=ti)
    sst_run2 = sst_run2.isel(time=i)
    diff = sst_obs - sst_run
    diff2 = sst_obs - sst_run2 
    ...
    ax2.contourf(sst_run['xh'],
                 ...
                 cmap=cmap,
                 extend='both')
    ax4.contourf(sst_obs['xh'],
                 sst_obs['yh'],
                 sst_obs,
                 levels=np.arange(tmin, tmax, .25),
                 cmap=cmap,
                 extend='both')
    ax5.contourf(sst_run2['xh'],
                 sst_run2['yh'],
                 sst_run2,
                 levels=np.arange(tmin, tmax, .25),
                 cmap=cmap,
