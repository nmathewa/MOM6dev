...
C
cd ~/HYCOM3-examples/GOMb0.08/expt_11.2
C
...
case 'HPE':
case 'XC30':
#                           substitute /p/work1 for /p/home
    setenv S `echo $cwd | awk '{print "/p/work1" substr($0,8,length)}'`
    breaksw
case 'AIX':
case 'XT5':
case 'IDP':
...
case 'XT4':
case 'OSF1':
#                           substitute /work for /usr/local/u
    setenv S `echo $cwd | awk '{print "/work" substr($0,13,length)}'`
    breaksw
case 'IRIX64':
#                           substitute /scr for /u/home
    setenv S `echo $cwd | awk '{print "/scr" substr($0,8,length)}'`
...
C
if ( `echo $Y01 | egrep 'csh' | wc -l` != 0 ) then
C
